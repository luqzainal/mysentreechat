...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect diuruskan oleh AuthProvider/MainLayout
    } catch (err) {
      setError(
        err.response?.data?.message || 'Gagal untuk log masuk. Sila cuba lagi.'
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="bg-white p-8 rounded-lg shadow-md space-y-8">
          ...
          {error && (
            <Alert variant="destructive" className="bg-red-100 border-red-400 text-red-700">
              <AlertTitle>Ralat Log Masuk</AlertTitle>
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <Button
                ...
                disabled={loading}
              >
                {loading ? 'Sedang diproses...' : 'Sign in'}
              </Button>
            </div>
          </form>
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <Link to="/register" className="font-medium text-indigo-600 hover:text-indigo-500">
               Register Now! {/* Tukar teks pautan ke pendaftaran */}
            </Link>
          </p>
        </div>
      </div>
